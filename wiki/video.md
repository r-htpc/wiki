...
Only use ALT: if you know what you're doing.
-->
...
2. Calibrate your display
  - Use specialized [hardware](https://calibrite.com/us/product/display-plus-hl/) and [software](https://store.portrait.com/consumer-software.html) means if you can afford it. Read [this article](https://tftcentral.co.uk/articles/lg-oled-tv-calibration-guide-autocal-and-hardware-calibration) for the basics
  - If you don't want to or can't do a hardware calibration, do a basic calibration of your display in your OS display settings. In Windows, use Start->Color Management->Advanced->Calibrate Display. You will have to adjust the settings on your display during this process. Brightness applies to blacks (crush). Contrast applies to whites (clipping). Color temperature and balance may also apply based on your preferences.
...
1. You may be using a bad, poor quality, or inadequately speced HDMI cable/adapter. Use a [better one](/wiki/components#video-cablesadapters). Don't use an adapter if not required.
...
4. Your Display and/or GPU is not supporting the full HDMI version spec or the resolution/refresh rate combination you're asking for. Read the manuals/specs for your display/sound system/gpu to find out what is supported. Follow our [setup](/wiki/video#setup-for-color-reproduction) steps. 
5. If for 4k@120Hz: WORKAROUND: Set your max HDMI bandwidth from 48 Gbps to 40 Gbps. Run [CRU](https://customresolutionutility.net/), set: \<Display\> -> Extension blocks -> CTA...(HDMI 2.1) -> Edit -> Data blocks -> HDMI 2.1 Support -> Edit -> Max FRL rate: 40 Gbps -> OK -> Reboot. *This should NOT be used as a permanent solution as a proper cable/port setup should not need this*
6. You may have a bad HDMI port on your Display, GPU, or an in-between Sound System. Try a different port.
7. You're using too many cable segments in the HDMI cable run from the HTPC to Display. You should limit the amount of segments to 2. HTPC->TV would be 1. HTPC->AVR->TV would be 2. HTPC->AVR->Wall jack->TV would be 3. If you have a wall jack behind the TV AND behind the AVR, that's 4. You can *usually* make up for using more segments with shorter/better cables or brush plates instead of wall jacks.
8. Your GPU driver may be corrupted. Run [DDU](https://www.guru3d.com/files-details/display-driver-uninstaller-download.html) to remove it fully and then re-install the driver.
9. You have your display/screen set to go to off after X amount of time in the OS. Disable this. Your display or sound system is turning your video port off after X amount of time. If there's no setting in the device to disable this, use [Unified Remote to configure a button to turn your monitor on/off](https://www.unifiedremote.com/remotes/monitor).
10. You're overclocking your GPU or have a bad GPU.
...
1080 @ 60 Hz:
 ...
 - Direct (LAN)
   - Cat 6-7 LAN + [Monoprice 4K+USB 2 Blackbird Extender](https://www.monoprice.com/product?p_id=44570) or [OREI 4K+USB 2 KVM Extender](https://www.amazon.com/Extender-Single-CAT5e-RS-232-Remote/dp/B07BC83WPM)
 - Remote
...
A GPU and Display with Display Stream Compression (DSC) support will allow more than the minimal native bandwidth of 32 Gbps. Displays that were made post-2022 or support >= 4K@144Hz usually have DSC support. iGPUs like Intel 11th+ Gen or AMD Ryzen 6000+ usually have DSC support.  
...
Use of an adapter in options #4/#5/#6/#7/#8 assumes you have a GPU with Displayport 1.4 protocol support and Display Stream Compression (DSC). 
...
2. DiY: An AMD Ryzen 8xxxG or Intel Core Ultra 2xx non-F desktop CPU and respective motherboard that states 4k@120Hz HDMI 2.1. e.g. Ryzen 8500G + [ASRock A620AI/B650I Lightning Wifi](https://pcpartpicker.com/products/motherboard/#f=8&s=41&sort=price) mobo/ASRock Deskmini X600, Intel Core 225 + [ASRock B860I Lightning](https://pcpartpicker.com/products/motherboard/#f=8&s=42&sort=price&page=1), for a realistic max of 4:2:2 10-bit HDR wo/DSC. 
